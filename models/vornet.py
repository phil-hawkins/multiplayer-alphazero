...
# Interface for defining a PyTorch model.
# See the models folder for examples.
class VorNetBN(Module):

    def __init__(self, input_shape, p_shape, v_shape):
        super(VorNetBN, self).__init__()
        self.input_shape = input_shape
        self.p_shape = p_shape
        self.v_shape = v_shape
        B, F = input_shape
        ...
        self.policy_head = MLP_BN(F, 256, 2, 1)
        self.value_head = MLP_BN(4*F, 512, 2, np.prod(self.v_shape))
...
class VorNetBN3(VorNetBN):
    def __init__(self, input_shape, p_shape, v_shape):
        ...
        self.input_bn = BatchNorm1d(F)
        self.policy_head = MLP_BN(F, 256, 3, 1)
        self.value_head = MLP_BN(4*F, 512, 3, np.prod(self.v_shape))